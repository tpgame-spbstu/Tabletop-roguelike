...
static func get_temp_map_config():
	...
	var loc2 = MapLocation.new(fight_scene, {"shuffle_seed": 1337, "ai_card_queue": card_queue1}, Vector2(1, 1))
...
static func get_temp_deck_config():
	...
	deck_config.cards.append(CardConfig.new(
		"Карта быстрая", Cost.new(3, 0, 0), ["fast"], [], 1, 2
		))
	deck_config.cards.append(CardConfig.new(
	...
	deck_config.cards.append(CardConfig.new(
		"Карта ослабляющая", Cost.new(2, 0, 0), ["weakening"], [], 1, 2
		))
	deck_config.cards.append(CardConfig.new(
		"Карта котратаки", Cost.new(3, 0, 0), ["counterattack"], [], 1, 3
		))
	deck_config.cards.append(CardConfig.new(
